...
# --------------------------------------------------------------------------
# A minimal histogram gradient booster, so this needs only numpy.
# --------------------------------------------------------------------------
class HistGBM:
    def __init__(self, n_estimators=200, learning_rate=0.06, max_depth=5,
                 n_bins=64, min_samples=40, seed=0):
        self.n_estimators, self.lr = n_estimators, learning_rate
        self.max_depth, self.n_bins = max_depth, n_bins
        self.min_samples, self.rng = min_samples, np.random.default_rng(seed)
    ...
    def _split(self, B, g, idx):
        """Best (feature, bin) split by squared-error gain, or None."""
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_estimators):
            tree = self._grow(B, y - pred, idx_all, self.max_depth)
            step = np.zeros(len(y))
            self._apply(B, tree, step, idx_all)
            pred += self.lr * step
